...
def find_ip_address(txt_3):
    ...
    while True:
        ...
        if txt_3[counter+1:counter+3].isnumeric():
            ...
            ip_address += txt_3[counter+1:counter_2+1]
            break
        else:
            counter += txt_3[counter+1:].find(" ") + 1
    return ip_address

def check_lost(txt_4, txt_5, txt_6, txt_7, ip_address):
    """check lost"""
    received = 4
    lost = 0
    if ip_address not in txt_4:
        received -= 1
        lost += 1
    if ip_address not in txt_5:
        received -= 1
        lost += 1
    if ip_address not in txt_6:
        received -= 1
        lost += 1
    if ip_address not in txt_7:
        received -= 1
        lost += 1
...
def main():
    ...
    ip_address = find_ip_address(txt_3)
    received, lost, percent_lost = check_lost(txt_4, txt_5, txt_6, txt_7, ip_address)
